...
def collimation_results(cf, cluster):
    '''Gather the results of collimation jobs and store in database'''
    info_sec = cf['info']
    mes_level = int(info_sec['mes_level'])
    log_file = info_sec['log_file']
    if len(log_file) == 0:
        log_file = None
    coll_path = info_sec['path']
    if not os.path.isdir(coll_path) or not os.listdir(coll_path):
        content = "There isn't result in path %s!" % coll_path
        utils.message('Warning', content, mes_level, log_file)
        return
    contents = os.listdir(coll_path)
    set_sec = cf['db_setting']
    db_info = cf['db_info']
    db = SixDB(db_info, set_sec, False, mes_level, log_file)
    file_list = utils.evlt(utils.decode_strings, [info_sec['outs']])
    where = "status='submitted'"
    job_ids = db.select('collimation_wu', ['wu_id', 'unique_id'], where)
    job_ids = [(str(j), str(i)) for i, j in job_ids]
    job_index = dict(job_ids)
    studypath = os.path.dirname(preprocess_path)
    unfin = cluster.check_running(studypath)
    running_jobs = [job_index.pop(unid) for unid in unfin]
    if running_jobs:
        content = "The collimation jobs %s aren't completed yet!" % str(
            running_jobs)
        utils.message('Warning', content, mes_level, log_file)
    ...
    for item in os.listdir(coll_path):
        if item not in job_index.values():
            continue
        job_path = os.path.join(coll_path, item)
        if not os.listdir(job_path):
            content = "The collimation job %s is empty!" % item
            utils.message('Warning', content, mes_level, log_file)
            continue
        job_table = {}
        task_table = {}
        ...
        if os.path.isdir(job_path) and os.listdir(job_path):
            ...
            task_id = task_id[0][0]
            rp.parse_collimation(item, job_path, file_list, task_table,
                    mes_level, log_file)
            where = 'task_id=%s' % task_id
            ...
            if task_table['status'] == 'Success':
                ...
                content = "Collimation job %s has completed normally!" % item
                utils.message('Message', content, mes_level, log_file)
            else:
                where = "wu_id=%s" % item
                job_table['status'] = 'incomplete'
            ...
            task_table['status'] = 'Failed'
            db.insert('collimation_task', task_table)
            content = "This is a failed job!"
            utils.message('Warning', content, mes_level, log_file)
        shutil.rmtree(job_path)
    db.close()
