...
def run(args):
    ...
    if num == 0:
        ...
        cf.read(input_file)
        madx_config = cf._sections['madx']
        madxjob(madx_config)

        sixtrack_config = cf._sections['sixtrack']
        sixtrackjobs(sixtrack_config)
    else:
        print("Too many input arguments!")
        sys.exit(1)

def madxjob(madx_config):
    '''MADX job to generate input file for sixtrack'''
    madxexe = madx_config["madx_exe"]
    source_path = madx_config["source_path"]
    jobname = madx_config["job_name"]
    seed = madx_config["seed"]
    jobname = jobname + "." + seed
    madxfile = os.path.join(source_path, jobname)
    dest_path = madx_config["dest_path"]
    
    #Begin to execute madx job
    command = madxexe + " " + madxfile
...
def sixtrackjobs(config):
    ...
    sixtrackjob(config, 'beta_oneturn', dp1='.000', dp2='.000')
...
def sixtrackjob(config, jobname, **args):
    '''One turn sixtrack job'''
    sixtrack_config = copy.deepcopy(config)
    ...
    c = fc3aux_2.split()
    lhc_length = c[4]
    sixtrack_config['length']=lhc_length
    sixtrack_config.update(args)
    ...
    keys = list(sixtrack_config.keys())
    patterns = ['%'+a for a in keys]
    values = list(sixtrack_config.values())
    output = []
